...
test.serial('deliver webhook', async (t) => {
  ...
  let callbackPromise = Sinon.promise<PromiseSettledResult<void>[]>()

  {
    ...
    await service.deliverRaw(eventType, payload, project, callbackPromise.resolve)
    ...
    t.is(callbackPromise.resolvedValue?.length, 1)
    t.is(callbackPromise.resolvedValue?.[0].status, 'fulfilled')

    callbackPromise = Sinon.promise()
  ...
  // timeout
  {
    ...
    t.true(stubSubscribe.calledOnce)
    t.is(callbackPromise.resolvedValue?.length, 1)
    t.is(callbackPromise.resolvedValue?.[0].status, 'rejected')

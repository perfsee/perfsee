...
export class DetailsRenderer {
  ...
  _renderBytes(details) {
    ...
    const value = Formatter.formatBytesToKiB(details.value, details.granularity || 0.1)
    const textEl = this._renderText(value)
    textEl.title = Formatter.formatBytes(details.value)
    return textEl
  }
  ...
  _renderMilliseconds(details) {
    ...
    if (details.displayUnit === 'duration') {
      value = Formatter.formatDuration(details.value)
    } else {
      value = Formatter.formatMilliseconds(details.value, details.granularity || 10)
    }
  ...
  _renderNumeric(details) {
    const value = Formatter.formatNumber(details.value, details.granularity || 0.1)
  ...
  _adornEntityGroupRow(rowEl) {
    ...
    if (matchedEntity.isFirstParty) {
      ...
      firstPartyChipEl.textContent = '1st party'
    ...
    if (matchedEntity.homepage) {
      ...
      entityLinkEl.title = 'Open in a new tab'

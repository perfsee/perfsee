...
export class NonStackProfile extends Profile {
  ...
  override forEachCallGrouped(
    _: (node: CallTreeNode, value: number) => void,
    __: (node: CallTreeNode, value: number) => void,
  ) {
  ...
  override forEachCall(
    ...
    function getAllEvents(node: NonStackTreeNode, events: { type: 'open' | 'close', node: NonStackTreeNode, time: number }[] = []) {

...
export function importMainThreadProfileFromChromeTimeline(events: TimelineEvent[]): Profile | undefined {
  const group = importFromChromeTimeline(events, 'chrome-profile')

  return group.profiles.find((profile) => profile.getName() === 'CrRendererMain') ?? group.profiles[0]
}

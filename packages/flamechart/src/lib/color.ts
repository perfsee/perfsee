...
export class Color {
  ...
  static fromCSSHex(hex: string) {
    ...
    const r = parseInt(hex.substring(1, 1 + 2), 16) / 255
    const g = parseInt(hex.substring(3, 3 + 2), 16) / 255
    const b = parseInt(hex.substring(5, 5 + 2), 16) / 255
    const a = hex.length === 9 ? parseInt(hex.substring(7, 7 + 2), 16) / 255 : 1

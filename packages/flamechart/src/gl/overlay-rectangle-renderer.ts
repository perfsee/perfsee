...
export class ViewportRectangleRenderer {
  private readonly material: Graphics.Material
  private readonly buffer: Graphics.VertexBuffer

  constructor(private readonly gl: Graphics.Context, theme: Theme) {
    const vertices = [
      [-1, 1],
      [1, 1],
      [-1, -1],
  ...
  render(props: ViewportRectangleRendererProps) {
    setUniformAffineTransform(this.material, 'configSpaceToPhysicalViewSpace', props.configSpaceToPhysicalViewSpace)
    ...
    this.gl.setBlendState(Graphics.BlendOperation.SOURCE_ALPHA, Graphics.BlendOperation.INVERSE_SOURCE_ALPHA)

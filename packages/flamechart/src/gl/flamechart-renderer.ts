...
class RangeTreeLeafNode implements RangeTreeNode {
  private readonly children: RangeTreeNode[] = []

  constructor(
    private readonly batch: RectangleBatch,
    private readonly bounds: Rect,
    private readonly numPrecedingRectanglesInRow: number,
  ) {}
...
class RangeTreeInteriorNode implements RangeTreeNode {
  private readonly rectCount = 0
  private readonly bounds: Rect
  constructor(private readonly children: RangeTreeNode[]) {
    ...
    this.bounds = new Rect(new Vec2(minLeft, minTop), new Vec2(maxRight - minLeft, maxBottom - minTop))
  }
...
export class FlamechartRenderer {
  private readonly layers: RangeTreeNode[] = []

  constructor(
    private readonly gl: Graphics.Context,
    private readonly rowAtlas: RowAtlas<FlamechartRowAtlasKey>,
    private readonly flamechart: Flamechart,
    private readonly rectangleBatchRenderer: RectangleBatchRenderer,
    private readonly colorPassRenderer: FlamechartColorPassRenderer,
    private readonly options: FlamechartRendererOptions = { inverted: false },
  ) {
    ...
    for (let stackDepth = 0; stackDepth < nLayers; stackDepth++) {
      ...
      for (let i = 0; i < layer.length; i++) {
        ...
        if (batch.getRectCount() >= MAX_BATCH_SIZE) {
          leafNodes.push(
            new RangeTreeLeafNode(batch, new Rect(new Vec2(minLeft, y), new Vec2(maxRight - minLeft, 1)), rectCount),
          )
          minLeft = Infinity
          maxRight = -Infinity
          batch = new RectangleBatch(this.gl)
        }
        const configSpaceBounds = new Rect(new Vec2(frame.start, y), new Vec2(frame.end - frame.start, 1))
  ...
  getRectInfoTexture(width: number, height: number): Graphics.Texture {
    if (this.rectInfoTexture) {
      ...
      this.rectInfoTexture = this.gl.createTexture(Graphics.TextureFormat.NEAREST_CLAMP, width, height)
    }
  ...
  getRectInfoRenderTarget(width: number, height: number): Graphics.RenderTarget {
    ...
    if (this.rectInfoRenderTarget && this.rectInfoRenderTarget.texture !== texture) {
  ...
  private readonly atlasKeys = new KeyedSet<FlamechartRowAtlasKey>()
  ...
  render(props: FlamechartRendererProps) {
    ...
    renderInto(this.gl, renderTarget, () => {
      ...
      const viewportRect = new Rect(Vec2.zero, new Vec2(this.gl.viewport.width, this.gl.viewport.height))

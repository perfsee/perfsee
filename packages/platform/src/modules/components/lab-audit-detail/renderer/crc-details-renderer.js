...
class CriticalRequestChainRenderer {
  ...
  static createChainNode(dom, segment, detailsRenderer) {
    ...
    segment.treeMarkers.forEach((separator) => {
      ...
      treeMarkeEl.append(dom.createElement('span', classSeparator), dom.createElement('span', 'lh-tree-marker'))
    })
    ...
    if (!segment.hasChildren) {
      ...
      span.textContent = ' - ' + Formatter.formatMilliseconds((endTime - startTime) * 1000) + ', '
      const span2 = dom.createElement('span', 'lh-crc-node__chain-duration')
      span2.textContent = Formatter.formatBytesToKiB(transferSize, 0.01)
  ...
  static render(dom, details, detailsRenderer) {
    ...
    // Fill in top summary.
    dom.find('.lh-crc-initial-nav', tmpl).textContent = 'Initial Navigation'
    dom.find('.lh-crc__longest_duration_label', tmpl).textContent = 'Maximum critical path latency:'
    dom.find('.lh-crc__longest_duration', tmpl).textContent = Formatter.formatMilliseconds(

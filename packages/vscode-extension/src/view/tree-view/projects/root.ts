...
export class ProjectsRootNodeDataProvider implements TreeNodeDataProvider<RootNode> {
  ...
  async getChildren(): Promise<TreeNode[]> {
    try {
      ...
      if (projects.length >= 1) {
        ...
        await commands.executeCommand('setContext', ContextValue.ProjectsViewWelcome, 'noProject')
        return []
      }
    } catch (err) {
      ...
      if (err instanceof UnauthorizedError) {
        await commands.executeCommand('setContext', ContextValue.ProjectsViewWelcome, 'login')
        return []
      }

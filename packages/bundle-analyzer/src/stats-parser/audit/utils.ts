...
export function rangeScore(num: number, good = 0, warn = 3): BundleAuditScore {
  return num <= good ? BundleAuditScore.Good : num <= warn ? BundleAuditScore.Warn : BundleAuditScore.Bad
}

export function getDomain(href: string) {
  if (href.startsWith('//') || /^https?:\/\//.test(href)) {
    const matches = /^(https?:)?(\/\/)?([^/]+)/.exec(href)
    return matches?.[3] ?? 'noop'
  }
 ...
 */
function hasPunctuatorBefore(content: string, startPosition: number) {
...
function computeTokenLength(content: string, features: { singlelineComments: boolean; regex: boolean }) {
   ...
   */
  const templateLiteralDepth: ('templateBrace' | 'normalBrace')[] = []
...
export function computeJSTokenLength(content: string) {
  return computeTokenLength(content, { singlelineComments: true, regex: true })
}

export function computeCSSTokenLength(content: string) {

...
export const SearchSelect = <T1 extends boolean = false, T2 extends SelectedKey = number>({
  ...
  useEffect(() => {
    if (multiSelect) {
      const selection = selectOptions && values && selectOptions.filter(({ key }) => values.includes(key))
      setShowText(selection?.map(({ text }) => text).join(', ') ?? '')
    }
  }, [values, multiSelect, selectOptions])
